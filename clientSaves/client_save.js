...
async function waitForAudio() { // we do not need such timeouts, later may make them larger
    if (audioDeque.length != 0) {
        ...
        let toAdd = await getData( new Blob([audioDeque.shift()], { type: "audio/wav" }));
  ...
  function handlerFunction(stream) {
    ...
    async function runRecorder(chunkInd) {
        recorders[chunkInd].ondataavailable = e => {
            ...
            if (stopped[chunkInd]) {
              //console.log(chunks[chunkInd]);
              //console.log(new Blob(chunks[chunkInd], { type: "audio/wav" }));

              //console.log(getData(new Blob(chunks[chunkInd], { type: "audio/wav" })), "###");
